...
async def read_all_proprietaires(db: db_dependency, user: user_dependency, request: Request):
    ...
    # check if user is agreed
    authorizeToView = ["opj", "apj", "apja"]
    if user.rp_qualif not in authorizeToView:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not enough permissions")
...
async def read_proprietaire(proprietaire_id: int, db: db_dependency, user: user_dependency, request: Request):
    ...
    api_log("proprietaires.read_one", level="INFO", request=request,email=user.email, user_id=user.id, proprietaire_id=proprietaire_id, tags=["proprietaires", "detail"], correlation_id=request.headers.get("x-correlation-id")) # type: ignore
    return proprietaire
...
async def create_proprietaire(proprietaire: proprietaireCreate, db: db_dependency, user: user_dependency, request: Request):
    # Check if user is agreed
    authorizeToModify = ["admin", "owner"]
    if user.privileges not in authorizeToModify:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not enough permissions")
